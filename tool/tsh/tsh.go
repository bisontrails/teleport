...
// cliPatch is used in tests to inject/override configuration within Run
type cliPatch func(*CLIConf) error

// Run executes TSH client. same as main() but easier to test
func Run(args []string, patches ...cliPatch) error {
	var cf CLIConf
	...
	// apply any patch options after parsing of arguments to ensure
	// that defaults don't overwrite patches.
	for _, patch := range patches {
		if err := patch(&cf); err != nil {
			return trace.Wrap(err)
		}
	}
